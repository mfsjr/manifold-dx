...
let onMutationCheckFailure = function<T>(baseline:T , test:T)  {
  let result = '*** MUTATION ERROR DETECTED!!! ***';
  console.log(result);
  return result;
};
...
describe("Add the name container", () => {
  ...
  describe("Verify StateMutationCheck", () => {
    ...
    test("swapping out the StateMutationCheck onFailure function", () => {
      testState.getManager().getActionProcessorAPI().setMutationCheckOnFailureFunction(onMutationCheckFailure);
