...
let resetTestObjects = () => {
  ...
  nameState = State.createStateObject<IName>(testState.getState() as ITestState & IStateObject, 'name', name);
  address = {street: '54 Upton Lake Rd', city: 'Clinton Corners', state: 'NY', zip: '12514'};
  addressState = State.createStateObject<IAddress>(nameState, 'address', address);
...
describe('mutate object values', () => {
  ...
  let stateNameContainer = _.get(testState.getState(), 'name') as IName & IStateObject;
  ...
  describe("Verify that mismatched property/container actions throw", () => {
    ...
    test("inserting a property should throw when a container is supplied", () => {
      expect(() => {mutateValue(ActionId.INSERT_PROPERTY, testState.getState() as ITestState & IStateObject, addressState, 'address')}).toThrow();
    })
  });
  ...
  describe("insert 'country' property, not part of IAddress", () => {
    ...
    test("delete the nameState from the name container", () => {
      let deleteResult = mutateValue(ActionId.DELETE_STATE_OBJECT, testState.getState() as ITestState & IStateObject, undefined, 'name');
